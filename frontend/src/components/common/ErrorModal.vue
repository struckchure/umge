  ...
  <div class="modal" :style="{ display: get_display }">
    <div class="modal-content">
      <div class="modal-header form-header flex-h-center space-around">
        ...
        <button
          class="flex-1 btn-small danger bg-img-none fntz-9 pa-1"
          @click="close_modal"
        >
          &times;
        </button>
      ...
      <div class="modal-body">
        <p v-for="(error, error_index) in get_error" :key="error_index">
          <label class="text-center text-black form-header">
            {{ error }}
          </label>
        </p>

        <p v-for="(success, success_index) in get_success" :key="success_index">
          <label class="text-center text-black form-header">
            {{ success }}
          </label>
        </p>
